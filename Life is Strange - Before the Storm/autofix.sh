...
if [ $EXTRACT -eq 1 ]; then
	...
	if [ $EXTRACT_ASSET_BUNDLES -eq 1 ]; then
		# I think this extension is specific to LiSBtS, and does *NOT*
		# imply that this is definitely an asset bundle. Asset bundles
		# tend to be under *_Data/StreamingAssets/Bundles, but may
		# appear elsewhere, e.g. in LiSBtS they also appear throughout
		# the DLC folder in the game's root, so it's not clear how to
		# identify these in a generic manner:
		find . -iname '*.bytes' -print0 | xargs -0 unity_asset_bundle_extractor.py
	fi
	cd extracted
