...
def load_3dmigoto_mesh(operator, paths):
    ...
    #vb.wipe_semantic_for_testing('POSITION.w', 0.0)
...
def import_normals_step1(mesh, data, vertex_layers):
    if len(data[0]) == 4:
        if [x[3] for x in data] != [0.0]*len(data):
            # Occurs in some meshes in DOA6, such as skirts.
            print('Normals are 4D, storing W component in NORMAL.w vertex layer')
...
def import_vertices(mesh, vb):
    ...
    for elem in vb.layout:
        ...
        #    use_normals = True
        #    import_normals_step1(mesh, data, vertex_layers)
        ...
        #    print('NOTICE: Skipping import of TANGENT in favour of recalculating on export')
        #elif elem.name.startswith('BINORMAL'):
        #    # Supposed to be the cross-product of the normal and tangent. DOA6
        #    # skirt meshes use this semantic, but I'm not sure they follow this
        #    # meaning.
        #    pass
...
def blender_vertex_to_3dmigoto_vertex(mesh, obj, blender_loop_vertex, layout, texcoords):
    ...
    for elem in layout:
        ...
        #    vertex[elem.name] = elem.pad(list(blender_loop_vertex.tangent), blender_loop_vertex.bitangent_sign)
            ...
            # use 4 dimensional positions and normals.
