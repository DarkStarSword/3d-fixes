...
def decode_cb_definition(file):
    ...
    print('ConstBuffer "{}" {}'.format(cb_name, cb_size))
    for j in range(num_entries):
        ...
        if type_size1 == 1:
            assert(type3 == 0)
            if type_size2 == 1:
                print('Float {} [{}]'.format(offset, entry_name))
            elif type_size2 == 4:
                print('Vector {} [{}]'.format(offset, entry_name))
            elif type_size2 == 3:
                print('Vector {} [{}] {}'.format(offset, entry_name, type_size2))
            else:
                raise ParseError('Unknown type_size2: {} for {}, type_size1: {}'.format(type_size2, entry_name, type_size1))
        elif type_size1 == 4:
            assert(type3 == 1)
            if type_size2 == 4:
                print('Matrix {} [{}]'.format(offset, entry_name))
            elif type_size2 == 3:
                print('Matrix {} [{}] {}'.format(offset, entry_name, type_size2))
            else:
...
def decode_bind(file):
    ...
    if bind_type == 0:
        ...
        print('SetTexture {} [{}] {} {}'.format(bind_slot, bind_name, texture_type, sampler_slot))
    elif bind_type == 1:
        assert(texture_type == 0)
        assert(sampler_slot == 0)
        print('BindCB "{}" {}'.format(bind_name, bind_slot))
    else:
        assert(False)
