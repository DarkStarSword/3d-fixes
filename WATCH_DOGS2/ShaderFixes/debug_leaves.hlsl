...
StructuredBuffer<struct SGraphicParticle> leaves : register(t110);
...
void main(point vs2gs input[1], inout TriangleStream<gs2ps> ostream)
{
	...
	emit_char(':', ostream);
	emit_char(' ', ostream);
	emit_float(leaves[idx].position.x, ostream);
	emit_char(' ', ostream);
	emit_float(leaves[idx].position.y, ostream);
	emit_char(' ', ostream);
	emit_float(leaves[idx].position.z, ostream);
}
