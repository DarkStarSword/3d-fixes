...
#ifdef VERTEX_SHADER
StructuredBuffer<float4x3> mSoftBodyNode : register(t10);
...
{
	uint len, stride;
	mSoftBodyNode.GetDimensions(len, stride);
	float4 pos = float4(0,0,0,1);

	output.node_idx_percent = (float)id / len;
	if (id < len) {
		output.pos = mul(float4(mul(pos, mSoftBodyNode[id]), 1), mW2P);
